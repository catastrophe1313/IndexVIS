...
var heatmap = function (date,type,sst,edd,ti,form) {
  var svg = null;
  ...
  var subtitle = '';
  var legendLabel = '';
  var width = document.getElementById("right21body").clientWidth-50;
  ...
  .text('')
    var width = document.getElementById("right21body").clientWidth-50;
  var height = document.getElementById("right21body").clientHeight-10;
    ...
    if (yAxisScale || yAxisLabels) {
      calculatedMargin.left = margin.left + 30;
    }
    ...
    svg = selection
      .append('svg')
      .attr('id','main')
      .attr('width', width+calculatedMargin.left )
      .attr('height', height+20 )
      .append('g')
      .attr('transform', 'translate(' + (calculatedMargin.left+10) + ',' + (calculatedMargin.top-10) + ')');
    ...
    if (yAxisScale || yAxisLabels) {
      if (yAxisScale) {
        ...
        .range([gridSizey/2, ((gridSizey) * rows)-gridSizey/2]);
    ...
    if (xAxisScale || xAxisLabels) {
      if (xAxisScale) {
        var x = d3.scaleTime()
        .domain([parseTime(date[0]),parseTime(date[date.length-1])])
        .range([gridSize/2, gridSize*40-gridSize/2]);

        svg.append('g')
          .attr('transform', 'translate('+(gridSize/2-5)+', '+(gridSize*type.length-10)+')')
          .attr('class', 'columnLabel axis')
          .call(axisBottom(x).tickFormat(d3.timeFormat('%Y/%m/%d')).ticks(20))
          .selectAll("text")
  ...
  var m=0.7-c//V-C
    ...
    svg.selectAll('g.column')
      .data(data)
      .enter().append('g')
      .each(function (d, i, j) { // function (d, i, j) might replace .each.
        select(this).selectAll('rect')
          .data(d)
          .enter().append('rect')
          .attr('x', function (d, j) { return (j * gridSize) + 3 }) // column
          .attr('y', function (d) { return i * (gridSizey) }) // row
          .attr('class', 'bordered')
          .attr('width', gridSize)
          .attr('height', gridSizey)
          .style('stroke', 'white')
          ...
          return tooltip1.style('visibility', 'visible').text("O " + ELIST[LLIST[type[i]]] + '\n'+ "date: " + date[j]+ '\n'+ "rate: " + (d*100).toFixed(2) + '%')
                        
                    
        })
         .on('click',function(d,j){
            CURRENT_DATE=date[j]
            CURRENT_NAME=LLIST[type[i]]
            AB_NAME=LLIST[type[i]]
            var idd = NAMELIST[LLIST[type[i]]]
            loll('总', CURRENT_DATE, GLOBAL_DATA, TB)
            // drawstack(AB_NAME, GLOBAL_DATA, ST, ED, NAMELIST, IDLIST, '#fff', CURRENT_DATE)
            drawstack(AB_NAME, GLOBAL_DATA, sst, edd, NAMELIST, IDLIST,'#fff',CURRENT_DATE);
            ...
            d3.select("#cd").text('Current Date : '+CURRENT_DATE);
