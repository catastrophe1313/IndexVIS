...
var height = document.getElementById("right22body").clientHeight-30;
    ...
    //         let yScale = d3.scaleLinear().domain([50, 250]).range([d.y1, d.y0])
    //         let xScale = d3.scaleLinear().domain([0, 358]).range([d.x0+10, d.x1-10])
    //         var line = d3.line()
    // .x(function(d,i){ return xScale(i)})
    // .y(function(d){ return yScale(d)})
    // .curve(d3.curveBasis)
    //         return line(d.data.line)} )
    //     .attr('stroke-width','2px')
    //     .attr('stroke','#ccc')
    //     .attr('fill','none')
    //     g.append('circle')
    //     .attr('cx',function(d,i){console.log(); let xScale = d3.scaleLinear().domain([0, 358]).range([d.x0+10, d.x1-10]) ;return xScale(i)})
    //     .attr('cy',function(d){ let yScale = d3.scaleLinear().domain([50, 250]).range([d.y1, d.y0]) ; console.log(yScale(d.data.line));return yScale(d.data.line)})
    //     .attr('r',2)
    //     .attr('fill','red')
    var da = []
